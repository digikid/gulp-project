...
│   │   ├── compress.js             # Объединение и минификация ресурсов в один файл
...
sourcemaps  | false | Создавать sourcemaps для JS и CSS файлов
minify  | false | Минифицировать файлы. Значения передаются через символ запятой `,` одной строкой (пример: `--minify css,js,html`).
compress  | false | Объединить и минифицировать все CSS / JS ресурсы в один файл (production mode)
babel  | false | Скомпилировать весь код в ES5, активируется только при флаге `--compress`
webp  | false | Конвертировать все изображения в формат WebP
main  | false | Загружать на сервер только основные файлы (по умолчанию: HTML, CSS и JS). Тип основных файлов задается в `config.paths.deploy.main`
open  | 'index' | Стартовая страница для запуска проекта в браузере
host  | 'default' | Ключ объекта с настройками FTP для загрузки файлов (из файла `ftp.js`)
mode  | 'dev' | Название режима. Переменная доступна глобально для HTML и JS файлов (@@mode)

Параметры по умолчанию задаются в файле `/gulp/defaults.js`.

> Обратите внимание, что параметр `--merge` не конкатенирует файлы библиотек с файлами проекта. Для запуска сборки в режиме production mode используйте параметр `--compress`.
...
> Если перед сборкой проекта передан параметр `--merge`, файлы из директорий `/src/styles/base` и `/src/styles/components` будут объединены в один файл.
...
> Если перед сборкой проекта передан параметр `--merge`, файлы из директорий `/src/javascripts/base` и `/src/javascripts/components` будут объединены в один файл.
...
После этого укажите в глобальном файле настроек `gulp/config.js` путь до файлов, которые необходимо импортировать (`config.paths.vendor`).
...
Для установки соединения и загрузки файлов на сервер запустите команду:
...
>Типы основных файлов задаются в глобальных настройках `config.paths.deploy.main`.
...
```
То же самое с последующей компиляцией кода в стандарт ES5 для поддержки устаревших браузеров:
...
**1.4.0**
- Добавлено: объединение и минификация ресурсов в один CSS / JS файл (production mode).
- Добавлено: файлы локализации для плагинов Fancybox и Air Datepicker.
- Добавлено: получение информации об устройстве, браузере и операционной системе.
- Добавлено: кластеризация маркеров в Яндекс.Картах.
- Обновлено: список полифиллов для IE.
- Исправлено: удаление неиспользуемого кода, исправление мелких ошибок.

**1.3.0**
- Обновлено: импорт библиотек напрямую из списка зависимостей.
- Удалено: файлы зависимостей удалены из исходных файлов.
- Исправлено: удаление неиспользуемого кода, исправление мелких ошибок.

**1.2.0**
- Добавлено: выборочная загрузка файлов на сервер.
- Исправлено: удаление неиспользуемого кода, исправление мелких ошибок.
